...
The normalized mean square error is used for both continuous and categorical variables. For continuous variables, it is equivalent to $1 - R^2$. For categorical variables, it is equivalent to $1 - BSS$ (Brier Skill Score or Ranked Probability Skill Score - cite Wilks?).

Wilks, D. S. (2005) Statistical Methods in the Atmospheric Sciences Chapter 7, ...
...
Brier Skill Score for each category (one-versus-all) and then averaged. Also for binary.


Variable type Metric                                                                                                      Metric 
------------- -----------------------------------------------------------------------------------                         -----------
Continuous    $\frac{\sum_{i=1}^{n}(x_i - \hat{x_i})^2}{\sum_{i=1}^{n} (x_i - \bar{x_i})^2}$                              $1 - R^2$     
Binary        $\frac{BS}{BSref}$, $BS =\frac{1}{N}\sum_{i=1}^{N}(x_i-p_i)^2$, $BSref =\frac{1}{N}\sum_{i=1}^{N}(x_i-p)^2$ $1 - BSS$
Categorical   $\frac{1}{K} \sum_{k=1}^{K}\frac{BS_k}{BSref_k}$, $K =$  no. of classes 

Ordinal variables are treated as categorical.

```{r}
# $\frac{\sum_{j=1}^{K}\frac{\sum_{i=1}^{n}(x_i-\hat{x_i})^2}{\sum_{i=1}^{n}(x_i-\bar{x_i})^2}}{K}$ 
```

half of the originally proposed Brier score: [@brier1950verification]
